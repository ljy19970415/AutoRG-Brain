...
from paths import *
from preprocess.cropping import get_case_identifier_from_npz
from experiment_planning_bucket.preprocessing_bucket import GenericPreprocessor

class ExperimentPlanner(object):
    def __init__(self, folder_with_cropped_data, preprocessed_output_folder, bucket_folder_with_cropped_data, bucket_preprocessed_output_folder):
        self.folder_with_cropped_data = folder_with_cropped_data
        self.preprocessed_output_folder = preprocessed_output_folder

        self.bucket_folder_with_cropped_data = bucket_folder_with_cropped_data 
        self.bucket_preprocessed_output_folder = bucket_preprocessed_output_folder

        self.list_of_cropped_npz_files = subfiles(self.folder_with_cropped_data, True, None, ".pkl", True)
        self.list_of_cropped_npz_files = list(filter(lambda x:not x.split('/')[-1].startswith('dataset_'),list(map(lambda x:self.bucket_folder_with_cropped_data+'//'+x.split('/')[-1].replace('.pkl','.npz'),self.list_of_cropped_npz_files))))

        # print("planner",self.list_of_cropped_npz_files)
    ...
    def run_preprocessing(self, num_threads):
        ...
        preprocessor.run(target_spacings, self.folder_with_cropped_data, self.preprocessed_output_folder, self.bucket_folder_with_cropped_data, self.bucket_preprocessed_output_folder,
