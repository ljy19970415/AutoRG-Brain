...
from preprocess.cropping import get_case_identifier_from_npz, ImageCropper
...
def resample_patient(data, seg, original_spacing, target_spacing, order_data=3, order_seg=0, force_separate_z=False,
                     ...
                     separate_z_anisotropy_threshold=RESAMPLING_SEPARATE_Z_ANISO_THRESHOLD,target_shape=None):
    """
    :param data:
    :param seg:
    ...
    new_shape = np.round(((np.array(original_spacing) / np.array(target_spacing)).astype(float) * shape)).astype(int) if target_shape is None else target_shape
...
class GenericPreprocessor(object):
    ...
    @staticmethod
    def load_cropped(cropped_output_dir, bucket_cropped_output_dir, case_identifier):

        # all_data = np.load(os.path.join(cropped_output_dir, "%s.npz" % case_identifier))['data']
        
        save_aws_image_path = bucket_cropped_output_dir+'//'+"%s.npz" % case_identifier
        value_buf = memoryview(client.get(save_aws_image_path))
        iostr = BytesIO(value_buf)
        all_data = np.load(iostr)['data']
    ...
    def resample_and_normalize(self, data, target_spacing, properties, seg=None, force_separate_z=None,target_shape=None):
        """
        ...
        data, seg = resample_patient(data, seg, np.array(original_spacing_transposed), target_spacing,
                                     ...
                                     separate_z_anisotropy_threshold=self.resample_separate_z_anisotropy_threshold, target_shape=target_shape)
        after = {
    ...
    def preprocess_test_case(self, data_files, target_spacing, seg_file=None, force_separate_z=None,target_shape=None):
        ...
        data, seg, properties = self.resample_and_normalize(data, target_spacing, properties, seg,
                                                            force_separate_z=force_separate_z,target_shape=target_shape)
        return data.astype(np.float32), seg, properties

    def _run_internal(self, target_spacing, case_identifier, output_folder_stage, cropped_output_dir, bucket_output_folder_stage, bucket_cropped_output_dir, force_separate_z,
                      all_classes):

        data, seg, properties = self.load_cropped(cropped_output_dir, bucket_cropped_output_dir, case_identifier)
        ...
        npz_file_path = os.path.join(output_folder_stage, "%s.npz" % case_identifier)
        np.savez_compressed(npz_file_path,data=all_data.astype(np.float32))

        print("saving: ", os.path.join(bucket_output_folder_stage, "%s.npz" % case_identifier))
        
        save_aws_image_path = bucket_output_folder_stage+'//'+ "%s.npz" % case_identifier
        os.system(f'aws s3 cp {npz_file_path} {save_aws_image_path}  --endpoint-url=http://10.140.14.204')

        os.remove(npz_file_path)
        print("remove: ", npz_file_path)

        with open(os.path.join(output_folder_stage, "%s.pkl" % case_identifier), 'wb') as f:
            pickle.dump(properties, f)
        
        #### delete the crop dir in the bucket at mean time ###
        save_aws_crop_npz_path = bucket_cropped_output_dir+'//'+"%s.npz" % case_identifier
        os.system(f'aws s3 rm {save_aws_crop_npz_path}  --endpoint-url=http://10.140.14.204')
        os.remove(os.path.join(cropped_output_dir, "%s.pkl" % case_identifier))

    def run(self, target_spacings, input_folder_with_cropped_npz, output_folder, bucket_input_folder_with_cropped_npz, bucket_output_folder, data_identifier,
        ...
        print("bucket npz folder:", bucket_input_folder_with_cropped_npz)
        print("output_folder:", output_folder)
        print("bucket output_folder", bucket_output_folder)

        # list_of_cropped_npz_files = subfiles(input_folder_with_cropped_npz, True, None, ".npz", True)

        list_of_cropped_npz_files = subfiles(input_folder_with_cropped_npz, True, None, ".pkl", True)
        list_of_cropped_npz_files = list(filter(lambda x:not x.split('/')[-1].startswith('dataset_'),list(map(lambda x:bucket_input_folder_with_cropped_npz+'//'+x.split('/')[-1].replace('.pkl','.npz'),list_of_cropped_npz_files))))
        ...
        for i in range(num_stages):
            ...
            bucket_output_folder_stage = bucket_output_folder + "//" + data_identifier + "_stage%d" % i
            ...
            for j, case in enumerate(list_of_cropped_npz_files):
                ...
                args = spacing, case_identifier, output_folder_stage, input_folder_with_cropped_npz, bucket_output_folder_stage,bucket_input_folder_with_cropped_npz,force_separate_z, all_classes
