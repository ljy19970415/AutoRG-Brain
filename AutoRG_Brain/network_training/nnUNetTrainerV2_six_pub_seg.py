...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def __init__(self, plans_file, fold, train_file, only_ana=False, abnormal_type="intense", num_batches_per_epoch=250, num_val_batches_per_epoch=50, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.val_every = 10
        self.val_choose_num = json.load(open('/mnt/petrelfs/leijiayu/nnUNet/AutoRG_Brain/utils_file/val_choose_number.json','r'))
        ...
        self.unpack_data = False ### has already unpacked data
        ...
        self.anatomy_reverse = anatomy_reverse
        self.reverse_anatomy_map = json.load(open('utils_file/hammer_label_reverse_map.json','r'))
    ...
    def do_split(self):
        ...
        if self.train_file is not None:
            ...
            val_keys = self.train_file["validation"]['SIX']
    ...
    def finish_online_evaluation(self, mode="train"):
        ...
        if mode == "train":
            ...
            self.all_val_eval_metrics_ana['SIX'].append(np.mean(global_dc_per_class_ana))
        ...
        if mode == "train":
            ...
            self.all_val_eval_metrics_ab['SIX'].append(np.mean(global_dc_per_class_ab))
    ...
    def run_iteration(self, data_generator, do_backprop=True, run_online_evaluation=False):
        ...
        if self.anatomy_reverse:
            target_anatomy_reverse = []
            for temp_anatomy in target_anatomy:
                temp2 = np.zeros(temp_anatomy.shape)
                for from_label, to_label in self.reverse_anatomy_map.items():
                    temp2[temp_anatomy == int(from_label)] = int(to_label)
                target_anatomy_reverse.append(temp2)
            target_anatomy = target_anatomy_reverse
