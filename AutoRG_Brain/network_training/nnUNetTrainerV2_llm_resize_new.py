...
class nnUNetTrainerV2(nnUNetTrainer):
    ...
    def __init__(self, plans_file, fold, train_file, only_ana=False, abnormal_type="intense", num_batches_per_epoch=250, num_val_batches_per_epoch=50, output_folder=None, dataset_directory=None, batch_dice=True, stage=None,
        ...
        self.train_file = json.load(open(train_file,'r'))
        self.test_file = self.train_file['validation']
        self.report = self.train_file['region_report']
    ...
    def initialize(self, training=True, force_load_plans=False, no_aug=False):
        ...
        if not self.was_initialized:
            ...
            self.plans = load_pickle('/mnt/petrelfs/leijiayu/nnUNet/AutoRG_Brain/utils_file/nnUNetPlansv2.1_plans_3D.pkl')
            self.process_plans(self.plans)

            ##### use the segmentation patch_size ####
            plans = load_pickle('/mnt/petrelfs/leijiayu/nnUNet/AutoRG_Brain/utils_file/nnUNetPlansv2.1_plans_3D.pkl')
    ...
    def get_tokenizer(self):
        ...
        checkpoint = "/mnt/petrelfs/leijiayu/envs/gpt-2-pubmed-medium"
    ...
    def initialize_network(self):
        ...
        if self.network_type == "normal":
            ...
            if self.use_global:
                ...
                self.network = Generic_UNet_share_patch_region(num_input_channels, base_num_features, 96, 2,
                                ...
                                net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
                                net_num_pool_op_kernel_sizes, net_conv_kernel_sizes, False, True, True,feature_layer=self.feature_layer, size=self.size)
                # self.network = Generic_UNet_share(num_input_channels, base_num_features, 96, 2,
                #                 len(net_num_pool_op_kernel_sizes),
                #                 conv_per_stage, 2, conv_op, norm_op, norm_op_kwargs, dropout_op,
                #                 dropout_op_kwargs,
                #                 net_nonlin, net_nonlin_kwargs, True, False, lambda x: x, InitWeights_He(1e-2),
                #                 net_num_pool_op_kernel_sizes, net_conv_kernel_sizes, False, True, True,feature_layer=self.feature_layer,avg_type=self.avg_type,pool_to_feature_layer=self.pool_to_feature_layer,use_conv_pool=self.use_conv_pool)
        ...
        # else:
        #     self.llm_model = LanguageModel(self.network.area_feature_dim)
