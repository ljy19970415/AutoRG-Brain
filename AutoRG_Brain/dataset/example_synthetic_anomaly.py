...
def get_shape(anatomy_scan, is_ellip):
    ...
    if is_ellip:
        ...
        out = nib.Nifti1Image(abnormal_mask.astype(np.int16), anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "ellipsode_shape.nii.gz")

        abnormal_mask = elasticdeform.deform_random_grid(abnormal_mask, sigma=sigma, points=3, order=0, axis=(0,1))

        out = nib.Nifti1Image(abnormal_mask.astype(np.int16), anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "ellipsode_shape_elastic.nii.gz")
    else:
        print("anatomy shape generation")
        ...
        out = nib.Nifti1Image(abnormal_mask.astype(np.int16), anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "anatomy_shape.nii.gz")
        abnormal_mask = scale_shape(abnormal_mask)
        out = nib.Nifti1Image(abnormal_mask.astype(np.int16), anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "anatomy_shape_scale.nii.gz")
        abnormal_mask = elasticdeform.deform_random_grid(abnormal_mask, sigma=sigma, points=3, order=0, axis=(0,1))
        out = nib.Nifti1Image(abnormal_mask.astype(np.int16), anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "anatomy_shape_elastic.nii.gz")
...
def pick_center(anatomy_scan):
    ...
    mask_scan = np.zeros(anatomy_scan.shape)
    mask_scan[anatomy_scan==22] = 1

    save_scan = np.zeros(anatomy_scan.shape)
    save_scan[anatomy_scan==22] = 1
    ...
    return xyz[0],xyz[1],xyz[2],save_scan

def find_position(edge_anatomy, center_anatomy, whole_brain, abnormal_mask, is_edge=True):
    ...
    if is_edge:
        cx,cy,cz, save_scan = pick_center(edge_anatomy)
        cx,cy,cz = 52, 25, 75
        # choose_ana = edge_anatomy[cx][cy][cz]
        # example_edge = np.zeros(edge_anatomy.shape)
        # example_edge[edge_anatomy==choose_ana] = 1
        out = nib.Nifti1Image(save_scan, anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "choose_ana_edge_mask.nii.gz")
    else:
        cx,cy,cz, save_scan = pick_center(center_anatomy)
        cx,cy,cz = 64, 45, 75
        # choose_ana = center_anatomy[cx][cy][cz]
        # example_center = np.zeros(center_anatomy.shape)
        # example_center[center_anatomy==choose_ana] = 1
        out = nib.Nifti1Image(save_scan, anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "choose_ana_center_mask.nii.gz")

    _,_,_, save_scan = pick_center(center_anatomy)
    out = nib.Nifti1Image(save_scan, anatomy_scan_affine, anatomy_scan_header)
    nib.save(out, "choose_ana_center_mask.nii.gz")
    
    print(edge_anatomy.shape)
    # print(cx,cy,cz)
...
def get_texture(brain_scan, abnormal_mask, whole_brain, intensity_dic, gap, modality, texture_intense):
    ...
    for idx,i in enumerate(layer):
        ...
        # without_blur = texture*temp

        abnormally_full_woblur = abnormally_full * (1-abnormal_mask) + abnormal_mask * texture
        out = nib.Nifti1Image(abnormally_full_woblur, anatomy_scan_affine, anatomy_scan_header)
        nib.save(out, "texture_woblur.nii.gz")
...
def GetSingleLesion(brain_scan, anatomy_scan, edge_anatomy, center_anatomy, whole_brain, modality, is_edge, is_ellip, texture_intense):

    abnormal_mask = get_shape(anatomy_scan, is_ellip)

    save_scan = np.zeros(anatomy_scan.shape)
    save_scan[anatomy_scan == 22] =1
    out = nib.Nifti1Image(save_scan, anatomy_scan_affine, anatomy_scan_header)
    nib.save(out, "choose_ana_mask.nii.gz")

    abnormal_mask, cx,cy,cz = find_position(edge_anatomy, center_anatomy, whole_brain, abnormal_mask, is_edge)
    
    intensity_dic, gap = get_intensity(brain_scan, anatomy_scan, modality)

    brain_scan, abnormal_mask = get_texture(brain_scan, abnormal_mask, whole_brain, intensity_dic, gap, modality, texture_intense)
    
    return brain_scan, abnormal_mask
...
def SynthesisTumor(brain_scan, anatomy_scan, modality, is_edge, is_ellip, texture_intense):
    ...
    brain_scan, abnormal_mask_anatomy = GetSingleLesion(brain_scan, anatomy_scan, edge_anatomy, center_anatomy, whole_brain, modality, is_edge, is_ellip, texture_intense)
    ...
    #     brain_scan, abnormal_mask_anatomy = GetSingleLesion(brain_scan, anatomy_scan, edge_anatomy, center_anatomy, whole_brain, modality)
    #     abnormal_mask = np.logical_or(abnormal_mask, abnormal_mask_anatomy)

    return brain_scan,abnormal_mask

brain_scan = nib.load("/mnt/petrelfs/leijiayu/nnUNet/AutoRG_Brain/utils_file/a02_image_0000.nii.gz")
anatomy_scan = nib.load("/mnt/petrelfs/leijiayu/nnUNet/AutoRG_Brain/utils_file/a02_image.nii.gz")
brain_scan_affine = brain_scan.affine
brain_scan_header =  brain_scan.header
anatomy_scan_affine = anatomy_scan.affine
anatomy_scan_header = anatomy_scan.header

if __name__=='__main__':

    # names = ['a02','a04','a15','a17','a18','a19']

    # for name in names:


    brain_data = brain_scan.get_fdata()
    anatomy_data = anatomy_scan.get_fdata()
    
    brain_data, anatomy_data = SynthesisTumor(brain_data, anatomy_data, 'T1WI', is_edge=False, is_ellip=False, texture_intense=1)

    abnormal_nii = nib.Nifti1Image(brain_data, brain_scan.affine, brain_scan.header)
    new_image_path = os.path.join('img_w_abnormal.nii.gz')
    nib.save(abnormal_nii, new_image_path)

    abnormal_nii = nib.Nifti1Image(anatomy_data, anatomy_scan.affine, anatomy_scan.header)
    new_image_path = os.path.join('abnormal_mask.nii.gz')
    nib.save(abnormal_nii, new_image_path)